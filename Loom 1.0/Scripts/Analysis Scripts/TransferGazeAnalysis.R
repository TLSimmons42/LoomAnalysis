...
tempdf <- df %>% filter(!is.na(P2B))


sd <-sd(tempdf$P2B)
tempdf <- df %>% filter(P2B < sd*3)
...
standardPlot <- tempdf %>%
  ...
  summarise(individualPoints = P2B,
            pointColor = pointColor,
            mATT = mean(P2B), sATT = sd(P2B),
...
#tempdf <- tempdf %>% filter(Group != "e")
tempdf <- tempdf %>% filter(Condition == "s")
t_test_result <- t.test(P2B ~ Group, data = tempdf , var.equal = FALSE)
print(t_test_result)
