...
for(f in 1:length(data_files))
{
  ...
  for(i in 1:nrow(trimedGrabDF))
  {
    ...
    subDFFull <- df %>% filter(Time < (currentTime+10000000) & Time > (currentTime-10000000))
    subDFFull <- subDFFull %>% filter(LeftPupil != -1)
    
    baseline <- mean(subDFpre$LeftPupil)
    ...
    if(nrow(subDFFull) != 0){
      ...
      subDFFull <- subDFFull %>%
        ...
        mutate(PercentChange = (LeftPupil - baseline) / baseline * 100)
