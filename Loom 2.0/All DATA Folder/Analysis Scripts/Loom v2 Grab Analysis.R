...
library(tidyr)   # <== THIS is what gives you pivot_wider()
library(lme4)
library(lmerTest)     # for p-values
library(effectsize)   # for eta squared effect sizes
library(emmeans)

setwd("C:/Users/Trent Simons/Desktop/Data/LoomAnalysis/Loom 2.0/All DATA Folder/Processed Data")


GrabmoveDF <- data.frame(Participant = factor(),
                     Condition = factor(),
                     Trial = numeric(),
                     Group = factor(),
                     startTime = numeric(),
...
for(f in 1:length(data_files))
{
  ...
  for (j in 1:nrow(Grab2DropDF)) {
    ...
    GrabmoveDF <- rbind(GrabmoveDF, newPartRow)
    
  }

  
  moveFiltDF <- GrabmoveDF %>% dplyr::filter(Participant == df$Participant[5])
  ...
  for (j in 1:nrow(moveFiltDF)) {
    ...
    # for (f in 1:nrow(GrabmoveDF)) {
    #   if(GrabmoveDF$startTime)
    # }
    
    GrabmoveDF <- GrabmoveDF %>% mutate(hand_time2Initiation = ifelse(startTime == moveFiltDF$startTime[j],handTime, hand_time2Initiation))
    GrabmoveDF <- GrabmoveDF %>% mutate(handRot_time2Initiation = ifelse(startTime == moveFiltDF$startTime[j],handRotTime, handRot_time2Initiation))
    GrabmoveDF <- GrabmoveDF %>% mutate(headRot_time2Initiation = ifelse(startTime == moveFiltDF$startTime[j],headTime, headRot_time2Initiation))
    GrabmoveDF <- GrabmoveDF %>% mutate(gaze_time2Initiation = ifelse(startTime == moveFiltDF$startTime[j],gazeTime, gaze_time2Initiation))
...
GrabmoveDF <- GrabmoveDF %>% mutate(Group = ifelse(Group == "c","Non-Aut",Group))
GrabmoveDF <- GrabmoveDF %>% mutate(Group = ifelse(Group == "e","Aut",Group))


#write.csv(GrabmoveDF, file = "C:/Users/Trent Simons/Desktop/Data/LoomAnalysis/Loom 2.0/All DATA Folder/SICK DATA FRAMES/Vel_and_Accel_df_6-10.csv", row.names = FALSE)
...
plotDF <- df %>% dplyr::filter(Time >= 638338370741730642  & Time <= 638338370744320656)
...
GrabmoveDF$Condition <- as.factor(GrabmoveDF$Condition)
GrabmoveDF$Group <- as.factor(GrabmoveDF$Group)


#GrabmoveDF <- grab2DropDF
GrabmoveDF <- GrabmoveDF %>% dplyr::filter(Condition != "")
...
useDF <- GrabmoveDF %>% filter(handRot_peak_accel <= sd(handRot_peak_accel)*3)
#useDF <- useDF %>% filter(cubeColor != "Gold")
#useDF <- useDF %>% filter(cubeColor == "White")

summary_df <- useDF %>%  
  group_by(Participant, Group, Condition, cubeColor) %>%
  summarize(mean_value = mean(handRot_peak_accel, na.rm = TRUE), .groups = "drop")



# # Step 2: Group-level summary with CI
# plot_df <- summary_df %>%
#   group_by(Group, Condition, cubeColor) %>%
#   summarize(
#     mean_metric = mean(mean_value),
#     sd = sd(mean_value),
#     n = n(),
#     se = sd / sqrt(n),
...
# Step 3: Plot — 1 plot per cubeColor, group as line
...
#   geom_errorbar(aes(ymin = mean_metric - ci95, ymax = mean_metric + ci95),
#                 width = 0.1, position = position_dodge(width = 0.2)) +
#   facet_wrap(~ cubeColor) +
#   labs(x = "Condition", y = "Mean Acceleration", color = "Group") +
#   theme_minimal()


process_measure <- function(data, measure_var, measure_label) {
  data %>%
    filter({{ measure_var }} <= sd({{ measure_var }}, na.rm = TRUE) * 3) %>%
    group_by(Participant, Group, Condition, cubeColor) %>%
    summarize(mean_value = mean({{ measure_var }}, na.rm = TRUE), .groups = "drop") %>%
    group_by(Group, Condition, cubeColor) %>%
    summarize(
      mean_metric = mean(mean_value),
      sd = sd(mean_value),
...
useDF1 <- GrabmoveDF %>% filter(gaze_avgVel <= sd(gaze_avgVel)*3)
useDF2 <- GrabmoveDF %>% filter(handRot_avgVel <= sd(handRot_avgVel)*3)
useDF3 <- GrabmoveDF %>% filter(handRot_peak_accel <= sd(handRot_peak_accel)*3)

# Process each measure
df_accel <- process_measure(useDF1, gaze_avgVel, "Avg Hand Velocity")
df_vel   <- process_measure(useDF2, handRot_avgVel,   "Avg Hand-Rotation Velocity")
df_dur   <- process_measure(useDF3, handRot_peak_accel,        "Avg Head-Rotation Velocity")


# Combine all into one
plot_df_all <- bind_rows(df_dur, df_accel, df_vel)

# Fix cubeColor ordering
plot_df_all$cubeColor <- factor(plot_df_all$cubeColor, levels = c("Still", "Slow", "Fast"))

plot_df_all$Measure <- factor(
  plot_df_all$Measure,
  levels = c("Avg Hand Velocity", "Avg Hand-Rotation Velocity", "Avg Head-Rotation Velocity"),
  labels = c("Avg Hand Velocity\n (m/s)", "Avg Hand-Rotation Velocity\n(degree/s²)", "Avg Head-Rotation Velocity\n(degree/s)")
)


ggplot(plot_df_all, aes(x = Condition, y = mean_metric, group = Group, color = Group)) +
  ...
  facet_grid(Measure ~ cubeColor, scales = "free_y", labeller = label_value) +  # <- FIXED
...
model_accel <- lmer(gaze_avgVel ~ Group * Condition * cubeColor + (1 | Participant),
                    data = useDF1)

# View model summary (with p-values)
summary(model_accel)

# Get effect sizes (partial eta squared for each fixed effect)
eta_squared(model_accel)

# Optional: Post-hoc pairwise comparisons by Group within each Condition
em_group <- emmeans(model_accel, ~ Group | Condition)
pairs(em_group, adjust = "bonferroni")
...
ggplot(GrabmoveDF, aes(x = Condition, y = Duration_Time, fill = Group)) +
  geom_boxplot() +
...
# GrabmoveDF <- GrabmoveDF %>% mutate(hand_time2PeakVel = hand_time2PeakVel/10000)
ggplot(GrabmoveDF, aes(x = Condition, y = gaze_avgVel, fill = Group)) +
  geom_boxplot() +
  labs(
    x = "Condition",
    y = "degrees/s",
...
ggplot(GrabmoveDF, aes(x = Condition, y = headRot_peak_vel, fill = Group)) +
...
ggplot(GrabmoveDF, aes(x = Condition, y = avg_accel, fill = Group)) +
  geom_boxplot(position = position_dodge(0.8), width = 0.7, outlier.shape = NA) +
  #geom_jitter(position = position_jitter(width = 0.2), size = 1, alpha = 0.6) +
  labs(x = "Condition", y = "speed",
...
ggplot(GrabmoveDF, aes(x = Condition, y = gaze, fill = Group)) +
  geom_boxplot(position = position_dodge(0.8), width = 0.7, outlier.shape = NA) +
  #geom_jitter(position = position_jitter(width = 0.2), size = 1, alpha = 0.6) +
  labs(x = "Condition", y = "speed",
